...
from home.enumerators import PRODUCT_CATEGORY, DRONE_BRAND, DRONE_CONDITION, DRONE_COLOR, SOLD_STATUS, PRODUCT_TYPE, \
    SPEED_MODE, WING_TYPE, DJANGO_VIEWS
    ...
    try:
        if request.method == 'POST':
            try:
                ...
                Data_Logger.log_error_message(DJANGO_VIEWS.CUSTOMER_PRODUCT, e, Data_Logger.get_client_ip(request), request.user)
        ...
        seo_tags = [{
            ...
            'description': "Manage your listed drone products with ease through the Customer Product Management "
                           "Panel at Dronza. Update, add, or delete products.",
        ...
        Data_Logger.log_error_message(DJANGO_VIEWS.CUSTOMER_PRODUCT, e, Data_Logger.get_client_ip(request))
